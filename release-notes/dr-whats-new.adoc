...
Per ulteriori informazioni sulla configurazione generale del proxy nella console NetApp , vedere https://docs.netapp.com/us-en/bluexp-setup-admin/task-configuring-proxy.html["Configurare l'agente della console per utilizzare un server proxy"^] .



=== Puoi terminare la prova gratuita in qualsiasi momento
...
Quando si verifica un errore e si avvia un failover, si esegue un failback o si testa il failover, è ora possibile visualizzare i dettagli delle VM e identificare quali VM non sono state riavviate.
...
Questa versione include il supporto per le VM montate su datastore VMware vSphere Virtual Machine File System (VMFS) per iSCSI e FC protetti su storage locale.  In precedenza, il servizio forniva un'anteprima tecnologica che supportava i datastore VMFS per iSCSI e FC.
...
. Dal menu, seleziona *Piani di replicazione*.
. Selezionare il piano di replica, fare clic sulla scheda *Failover mapping* e fare clic sull'icona della matita *Modifica*.
. Fare clic sulla freccia *Datastore* per espanderla.
. Prendere nota del valore del conteggio di conservazione nel piano di replica.  Una volta completati questi passaggi, sarà necessario ripristinare il valore originale.
. Ridurre il conteggio a 1.
. Avvia uno snapshot su richiesta.  Per farlo, nella pagina Piano di replica, seleziona il piano, fai clic sull'icona Azioni e seleziona *Esegui snapshot ora*.
. Dopo aver completato correttamente il processo di snapshot, riporta il conteggio nel piano di replica al valore originale annotato nel primo passaggio.
...
È possibile aggiungere un piano di replica più facilmente filtrando le VM in base al datastore nella pagina Applicazioni e selezionando maggiori dettagli sulla destinazione nella pagina Mappatura risorse. Fare riferimento a https://docs.netapp.com/us-en/bluexp-disaster-recovery/use/drplan-create.html["Creare un piano di replicazione"] .
...
Le informazioni sui prerequisiti di NetApp Disaster Recovery sono state aggiornate.
...
Per i dettagli sulla gestione delle licenze per *tutti* i servizi dati della console NetApp , fare riferimento a https://docs.netapp.com/us-en/bluexp-digital-wallet/task-manage-data-services-licenses.html["Gestisci le licenze per tutti i servizi dati della console NetApp"^] .
...
Ora puoi cliccare sulle opzioni nella Dashboard, così potrai rivedere le informazioni più facilmente e rapidamente. Inoltre, la Dashboard ora mostra lo stato dei failover e delle migrazioni.
