...
[role="lead"]
Dopo aver aggiunto risorse e siti, sei pronto per creare un . I piani di replica gestiscono la protezione dei dati. La creazione della replica prevede la designazione dei siti di origine e di destinazione, la selezione dei gruppi di risorse e la scelta di come le applicazioni devono essere ripristinate e avviate. Ad esempio, potresti raggruppare le macchine virtuali (VM) associate a un'applicazione oppure potresti raggruppare applicazioni che hanno livelli simili.
...
. Per *Origine*, selezionare il vCenter o cluster Kubernetes da designare come origine.
...
** Se il gruppo di risorse e le risorse associate devono essere trasferite al sito di disaster recovery, tutte le risorse non protette non saranno più presenti nel sito di origine a seguito del processo di failover, con conseguente arresto anomalo delle VM non protette nel sito di origine. Inoltre, NetApp Disaster Recovery non avvierà tali VM non protette nel sito di failover vCenter.
