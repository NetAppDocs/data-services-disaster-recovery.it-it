...
[role="lead"]
Dopo aver aggiunto risorse e siti, crea un piano di replica per proteggere i tuoi dati. Il piano definisce i siti di origine e di destinazione, i gruppi di risorse, le mappature di recovery e il comportamento all'avvio per le tue applicazioni. Puoi anche configurare la conformità e le pianificazioni dei test ed eseguire failover di prova senza influire sui dati di produzione.
...
Puoi proteggere più VM su più datastore. NetApp Disaster Recovery crea gruppi di coerenza ONTAP per tutti i volumi ONTAP che ospitano i datastore delle VM protette.
...
** Se mancano degli snapshot sul sito di destinazione, il servizio potrebbe eliminare gli snapshot corrispondenti sul sito di origine durante il successivo processo di riconciliazione degli snapshot programmato, a seconda della policy SnapMirror predefinita per la relazione.
...
=== Considerazioni sui backup gestiti dalla piattaforma

Se prevedi di utilizzare backup gestiti dalla piattaforma (cioè snapshot gestiti da ONTAP), ti consigliamo di usare la policy di replica *MirrorAll*. Consulta la tabella per capire l'impatto delle diverse policy di replica SnapMirror e perché MirrorAll è l'opzione consigliata.

[cols="10,10,80"]
|===


| Politica | Raccomandazione | Note 


| *MirrorAll* | Consigliato | Non utilizzare gli snapshot di trasferimento SnapMirror per operazioni di failover o failback. 


| *MirrorVault* .2+| Raccomandazione alternativa .2+| Assicurati che gli snapshot selezionati durante il failover o il failback siano presenti sia sul volume di origine che su quello di destinazione. Se non lo fai, verrà visualizzato l'errore: "Transizione allo stato 'snapmirrored' non riuscita. Motivo: nessuno snapshot comune trovato." Non utilizzare gli snapshot di trasferimento SnapMirror per operazioni di failover o failback. 


| *Asincrono* 


| *MirrorLatest* | NON consigliato | Questa policy fa sì che la destinazione disponga di fatto di un solo snapshot comune per il failover, il che è subottimale per gli scenari di disaster recovery. 
|===


[[considerations-for-disaster-recovery-managed-backups]]
=== Considerazioni sui backup gestiti da NetApp Disaster Recovery

Quando usi i backup gestiti di NetApp Disaster Recovery, le seguenti linee guida si applicano alle relazioni di SnapMirror create al di fuori di NetApp Disaster Recovery:

* Le relazioni di SnapMirror possono supportare le policy di replica MirrorAll, MirrorVault, MirrorLatest e asincrona.
* Non programmare aggiornamenti per queste relazioni, poiché saranno gestite dal servizio NetApp Disaster Recovery.
...
TIP: Organizza le tue VM o i cluster Kubernetes prima di implementare NetApp Disaster Recovery per ridurre al minimo la proliferazione. Posiziona le risorse che necessitano di protezione su un sottoinsieme di datastore e le risorse che non devono essere protette su un sottoinsieme diverso di datastore. Per organizzare i gruppi di risorse prima di creare il piano di replica, consulta link:resource-group.html["Crea un gruppo di risorse"].
...
La procedura guidata ti guida attraverso questi passaggi:
...
* Non ospitare risorse protette e non protette sullo stesso datastore. Se risorse protette e non protette sono ospitate sullo stesso datastore, puoi riscontrare questi problemi:
+
...
** Se il gruppo di risorse e le risorse associate devono essere trasferite al sito di disaster recovery, tutte le risorse non protette non esisteranno più nel sito di origine a seguito del processo di failover, con conseguente fallimento delle risorse non protette nel sito di origine. Inoltre, NetApp Disaster Recovery non avvierà tali risorse non protette nel sito di failover.





TIP: Crea un set separato di mappature dedicate per i tuoi test di failover, in modo da impedire che le risorse vengano connesse alle reti di produzione utilizzando gli stessi indirizzi IP.
...
+
** Per rimuovere una VM o un datastore, passa il mouse sul nome della risorsa e poi seleziona *X*.
...
+
Se non disponi di gruppi di risorse esistenti o devi aggiungere risorse a uno, seleziona *+ Crea nuovo gruppo di risorse*. Segui le istruzioni per link:resource-group.html["aggiungere spazi dei nomi a un gruppo di risorse"].
...
TIP: Eseguire il comando CLI  `kubectl get pvc -n <namespace>` sul namespace per visualizzare la storage class.
...
+
Quando imposti la frequenza degli snapshot, devi scegliere una frequenza di almeno ogni cinque minuti.
...
+
Per Azure, scegli *Sottoscrizione*, *Gruppo di risorse* e *Account di storage* per il cloud provider.

+
Per AWS, ONTAP S3 e StorageGRID, scegli *Acount*, *Credentials* e *Bucket*.
...
.. Seleziona **Aggiungi**.
+
image:kubernetes-mapping.png["Screenshot della mappatura della Storage class per Kubernetes"]
...
==== Trasformazioni delle risorse ed esecuzioni di script

Per i piani di replica di Kubernetes, puoi applicare trasformazioni delle risorse ed esecuzioni di script. Questi passaggi sono facoltativi.

.Aggiungi una trasformazione di risorse
Aggiungere una trasformazione delle risorse ti permette di modificare qualsiasi aspetto di un carico di lavoro durante il failover. Le trasformazioni delle risorse offrono flessibilità per adattare i carichi di lavoro a diversi ambienti e configurazioni durante un evento di disaster recovery. link:task-resource-transformation.html["Scopri come creare e gestire le trasformazioni delle risorse."^]

Le trasformazioni delle risorse vengono applicate nell'ordine in cui le elenchi.

. Dal menu a tendina, seleziona la trasformazione delle risorse che vuoi aggiungere.
. Scegli a quali risorse vuoi applicare la trasformazione e poi aggiungi i valori richiesti
. Rivedi l'ordine delle trasformazioni per assicurarti che le operazioni vengano eseguite nell'ordine corretto.


.Aggiungi l'esecuzione di uno script
Disaster Recovery può eseguire uno script prima o dopo la creazione di uno snapshot per gestire la quiescenza e la ripresa di un'applicazione. Eseguendo questa operazione, ti assicuri che le applicazioni siano adeguatamente preparate prima che uno snapshot venga replicato e ripristinato e possano riprendere le operazioni senza interruzioni dopo un failover.

. Seleziona *+ Aggiungi script*.
. Seleziona *Carica* per scegliere il file dello script.
. Scegli la *fase di esecuzione*: *Pre-snapshot* per eseguire prima della creazione dello snapshot oppure *Post-snapshot* per eseguire dopo.
. Seleziona *Salva*.
...
La sezione Risorse di calcolo definisce dove vengono ripristinate le macchine virtuali dopo il failover. Mappa il data center e il cluster di origine vCenter a un data center e cluster di destinazione.

Facoltativamente, le macchine virtuali possono essere riavviate su uno specifico vCenter ESXi host. Se VMware DRS è abilitato, puoi spostare automaticamente la macchina virtuale su un host alternativo, se necessario, per soddisfare la policy di DR configurata.

Facoltativamente, puoi inserire tutte le VM di questo piano di replica in una cartella univoca all'interno dell'inventario di vCenter. Questo ti offre un modo semplice per organizzare rapidamente le VM di failover all'interno di vCenter.
...
NOTE: Questa sezione è richiesta solo per vCenters. Non è necessario completarla per Kubernetes.
...
Nella scheda Mappature di failover, seleziona la freccia rivolta verso il basso accanto a *Reti virtuali*. Seleziona la rete virtuale di origine e la rete virtuale di destinazione.
...
NOTE: Attiva le *Impostazioni avanzate* per configurare gli *Script* e la *coerenza con l'applicazione*.
...
* *Tipo di indirizzo IP*: Riconfigura la configurazione della VM in modo che corrisponda ai requisiti della rete virtuale di destinazione. NetApp Disaster Recovery offre due opzioni: DHCP o IP statico. Per gli IP statici, configura la subnet mask, il gateway e i server DNS. Inoltre, inserisci le credenziali per le VM.
+
...
+
*VM con Microsoft Windows*: puoi eseguire sia script batch di Windows (.bat) sia script PowerShell (ps1). Gli script di Windows possono usare argomenti da riga di comando. Formatta ogni argomento nel formato  `arg_name$value`, dove  `arg_name` è il nome dell'argomento e  `$value` è il valore dell'argomento, e un punto e virgola separa ogni coppia  `argument$value`.
...
+
** Avvio sequenziale: assegna a ciascuna macchina virtuale un numero univoco per avviarle nell'ordine prestabilito, ad esempio 1,2,3,4,5.
...
I dati gestiti dal database server potrebbero trovarsi sullo stesso datastore della VM che ospita il database server oppure su un datastore diverso. La tabella seguente mostra le configurazioni supportate per gli snapshot coerenti con l'applicazione in NetApp Disaster Recovery:
...
* Per le macchine virtuali Windows, il servizio utilizza il Microsoft Volume Shadow Copy Service (VSS) per coordinarsi con entrambi i tipi di database server.
...
TIP: Esamina attentamente le informazioni <<Considerazioni sui backup gestiti dalla piattaforma>> prima di effettuare queste selezioni.

* *Utilizza backup e pianificazioni di conservazione gestiti dalla piattaforma*: se usi una soluzione esterna per la gestione degli snapshot, seleziona questa casella. Il Disaster Recovery supporta l'utilizzo di soluzioni esterne per la gestione degli snapshot, come il policy scheduler nativo di ONTAP SnapMirror o integrazioni di terze parti. Se ogni datastore (volume) nel piano di replica ha già una relazione di SnapMirror gestita altrove, puoi usare quegli snapshot come punti di ripristino in NetApp Disaster Recovery.
+
Quando questa opzione è selezionata, NetApp Disaster Recovery non configura una pianificazione di backup. Tuttavia, devi comunque configurare una pianificazione di conservazione perché potrebbero comunque essere creati snapshot per operazioni di test, failover e failback.
...
* *Archivi dati di origine e di destinazione*: se esistono più relazioni (fan-out) SnapMirror, puoi selezionare la destinazione da utilizzare. Se un volume ha già una relazione di SnapMirror stabilita, vengono visualizzati gli archivi dati di origine e di destinazione corrispondenti. Se un volume non ha una relazione di SnapMirror, puoi crearne una ora selezionando un cluster di destinazione, selezionando una SVM di destinazione e specificando un nome per il volume. Il servizio creerà il volume e la relazione di SnapMirror.
+
...
** Durante il failover, puoi selezionare uno snapshot qualsiasi. Se selezioni lo snapshot più recente, il servizio crea un backup su richiesta, aggiorna la destinazione e utilizza quello snapshot per il failover.
