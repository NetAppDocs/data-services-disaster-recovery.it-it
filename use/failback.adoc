---
sidebar: sidebar 
permalink: use/failback.html 
keywords: disaster recovery, bluexp disaster recovery, failover, failback, replicate, fail over, fail back, vmware, vcenter 
summary: NetApp Disaster Recovery è un servizio di disaster recovery basato su cloud che automatizza i flussi di lavoro di disaster recovery. 
---
= Ripristina le applicazioni alla fonte originale con NetApp Disaster Recovery
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ../media/use/


[role="lead"]
Dopo aver risolto un disastro, eseguire il failback dal sito di disaster recovery al sito di origine per ripristinare le normali operazioni.  È possibile selezionare lo snapshot da cui effettuare il ripristino.

*Ruolo obbligatorio NetApp Console * Ruolo di amministratore dell'organizzazione, amministratore di cartelle o progetti, amministratore del ripristino di emergenza o amministratore del failover del ripristino di emergenza.

link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-disaster-rec-roles.html["Scopri di più sui ruoli utente e sulle autorizzazioni in NetApp Disaster Recovery"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Scopri di più sui ruoli di accesso NetApp Console per tutti i servizi"^].



== Informazioni sul failback

In caso di failback, NetApp Disaster Recovery replica (risincronizza) tutte le modifiche sulla macchina virtuale di origine prima di invertire la direzione della replica. Questo processo inizia da una relazione che ha completato il failover verso una destinazione e prevede i seguenti passaggi:

* Eseguire un controllo di conformità sul sito recuperato.
* Aggiornare le informazioni vCenter per ogni cluster vCenter identificato come situato nel sito ripristinato.
* Nel sito di destinazione, spegnere e annullare la registrazione delle macchine virtuali e smontare i volumi.
* Interrompere la relazione SnapMirror sulla sorgente originale per renderla di lettura/scrittura.
* Risincronizzare la relazione SnapMirror per invertire la replica.
* Accendere e registrare le macchine virtuali di origine, quindi montare i volumi sull'origine.




== Prima di iniziare

Se si utilizza la protezione basata su datastore, le VM aggiunte al datastore potrebbero essere aggiunte al datastore durante il processo di failover. In tal caso, assicurarsi di fornire informazioni di mappatura aggiuntive per queste VM prima di avviare il failback. Per modificare la mappatura delle risorse, vedere link:manage.html#manage-replication-plans["Gestire i piani di replicazione"].



== Passi

[role="tabbed-block"]
====
.vCenter
--
.Passi
. Dal menu NetApp Disaster Recovery , selezionare *Piani di replica*.
. Nel menu a tendina, imposta il filtro su *vCenter* piani di replica.
+
image::site-type-toggle.png[Screenshot dell'interruttore del tipo di sito]

. Identifica il piano di replica di cui desideri eseguire il failback. Seleziona l'opzione *Azioni* image:../use/icon-horizontal-dots.png["Icona del menu Azioni nel servizio NetApp Disaster Recovery"], posiziona il cursore su *Ripristina* e quindi seleziona *Failback*.
. Scegli lo snapshot del datastore da cui eseguire il ripristino. Seleziona *Crea snapshot ora* per avviare un nuovo snapshot oppure *Seleziona* per scegliere tra gli snapshot disponibili.
+
image:failback.png["Screenshot della finestra di dialogo di failback"]

. Immettere "Failback" per confermare l'operazione, quindi selezionare *Failback*.


Per monitorare l'avanzamento del processo, selezionare *Monitoraggio processo* nel menu Disaster Recovery.

--
.Kubernetes
--
. Dal menu NetApp Disaster Recovery , selezionare *Piani di replica*.
. Nel menu a tendina, filtra per piani di replica *Kubernetes*.
+
image::site-type-toggle.png[Screenshot dell'interruttore del tipo di sito]

. Identifica il piano di replica di cui desideri eseguire il failback. Seleziona l'opzione *Azioni* image:../use/icon-horizontal-dots.png["Icona del menu Azioni nel servizio NetApp Disaster Recovery"], posiziona il cursore su *Ripristina* e quindi seleziona *Failback*.
. Scegli lo snapshot del datastore da cui eseguire il ripristino. Seleziona *Crea snapshot ora* per avviare un nuovo snapshot oppure *Usa lo snapshot più recente* per utilizzare lo snapshot più recente e riuscito.
. Immettere "Failback" per confermare l'operazione, quindi selezionare *Failback*.


Per monitorare l'avanzamento del processo, selezionare *Monitoraggio processo* nel menu Disaster Recovery.

--
====