...
[role="lead"]
Il disaster recovery nel cloud è un modo resiliente e conveniente per proteggere i carichi di lavoro da interruzioni del sito e da eventi di corruzione dei dati. Con NetApp Disaster Recovery, puoi replicare i carichi di lavoro delle VM VMware o dei datastore on-premises che utilizzano storage ONTAP verso un data center VMware software-defined in un cloud pubblico utilizzando NetApp cloud storage oppure verso un altro ambiente VMware on-premises con storage ONTAP come sito di disaster recovery. Puoi utilizzare Disaster Recovery anche per migrare i carichi di lavoro delle VM da un sito a un altro.

NetApp Disaster Recovery è un servizio di disaster recovery basato sul cloud che automatizza i flussi di lavoro di ripristino d'emergenza. Con NetApp Disaster Recovery, puoi proteggere i tuoi cluster Kubernetes on-premises, i carichi di lavoro basati su NFS e i datastore del file system della macchina virtuale VMware vSphere (VMFS) per iSCSI e FC che utilizzano lo storage NetApp verso una delle seguenti destinazioni:
...
Il Disaster Recovery utilizza i seguenti componenti per gestire i carichi di lavoro di Disaster Recovery:
