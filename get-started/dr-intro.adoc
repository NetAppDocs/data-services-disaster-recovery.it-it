...
summary: Il disaster recovery nel cloud è un modo resiliente e conveniente per proteggere i carichi di lavoro da interruzioni del sito e eventi di corruzione dei dati. Con NetApp Disaster Recovery, puoi replicare i carichi di lavoro VMware on-premises che utilizzano storage ONTAP verso un data center VMware software-defined in un cloud pubblico utilizzando NetApp cloud storage oppure verso un altro ambiente VMware on-premises con storage ONTAP come sito di disaster recovery. 
---
...
[role="lead"]
Il disaster recovery nel cloud è un modo resiliente e conveniente per proteggere i carichi di lavoro da interruzioni del sito e da eventi di corruzione dei dati. NetApp Disaster Recovery è un servizio di disaster recovery basato sul cloud che automatizza i flussi di lavoro di disaster recovery. Con NetApp Disaster Recovery, puoi proteggere i tuoi cluster Kubernetes on-premises, i carichi di lavoro basati su NFS e i datastore del file system della macchina virtuale VMware vSphere (VMFS) per iSCSI e FC che utilizzano lo storage NetApp verso una delle seguenti destinazioni:

* Amazon Elastic VMware Service (EVS) con Amazon FSx for NetApp ONTAP. Per dettagli, consulta link:../reference/evs-deploy-guide-introduction.html["Introduzione al NetApp Disaster Recovery con Amazon Elastic VMware Service e Amazon FSx per NetApp ONTAP"].
* VMware Cloud (VMC) su AWS con Amazon FSx for NetApp ONTAP
* Azure VMware Solution con NetApp Cloud Volumes ONTAP (iSCSI) (Anteprima privata)
* Google Cloud VMware Engine (GCVE) con Google Cloud NetApp Volumes
* Un altro ambiente VMware locale basato su NFS e/o VMFS (iSCSI/FC) con storage ONTAP
...
image:../use/evs-console-architecture.png["Diagramma che mostra i componenti del NetApp Disaster Recovery"]
...
== Vantaggi dell'utilizzo di NetApp Disaster Recovery
...
== Capacità di NetApp Disaster Recovery
...
* Eseguire un test di failover. Quando si esegue questa operazione, il servizio crea macchine virtuali temporanee. NetApp Disaster Recovery crea un nuovo volume FlexClone dallo snapshot selezionato e un datastore temporaneo, che è supportato dal volume FlexClone, viene mappato agli host ESXi. Questo processo non consuma ulteriore capacità fisica sullo storage ONTAP locale o su FSx for NetApp ONTAP storage in AWS. Il volume di origine originale non viene modificato e i processi di replica possono continuare anche durante il disaster recovery.
...
image:../use/evs-use-cases.png["Diagramma che illustra i casi d'uso per NetApp Disaster Recovery"]


NOTE: È necessario configurare il server vSphere al di fuori di NetApp Disaster Recovery in vSphere Server.
...
* Acquista un abbonamento pay-as-you-go (PAYGO) tramite Amazon Web Services (AWS) Marketplace, Azure Marketplace o Google Cloud Marketplace. Questa licenza ti permette di acquistare una licenza a capacità protetta fissa senza alcun impegno a lungo termine.
...
Disaster Recovery viene concesso in licenza in base alla quantità di dati ospitati sui volumi ONTAP protetti. Il servizio determina quali volumi devono essere considerati ai fini della licenza mappando le VM protette ai relativi datastore vCenter. Ogni datastore è ospitato su un volume ONTAP o su una LUN. La capacità utilizzata riportata da ONTAP per quel volume o LUN viene utilizzata per la determinazione della licenza.
...
NOTE: NetApp Disaster Recovery si basa sulla capacità utilizzata dei datastore nel sito di origine quando è presente almeno una VM o una risorsa Kubernetes con un piano di replica. La capacità di un datastore in failover non è inclusa nel limite di capacità. Per una BYOL, se i dati superano la capacità consentita, le operazioni nel servizio sono limitate finché non ottieni una licenza di capacità aggiuntiva o non aggiorni la licenza nella NetApp Console.
...
Puoi provare NetApp Disaster Recovery con una prova gratuita di 30 giorni.

Per continuare dopo il periodo di prova di 30 giorni, è necessario ottenere un abbonamento Pay-as-you-go (PAYGO) dal proprio cloud provider o acquistare una licenza BYOL da NetApp.
...
image:dr-architecture-diagram-70-2.png["Diagramma che illustra l'architettura del servizio di ripristino d'emergenza NetApp infrastructure"]
...
NetApp Disaster Recovery utilizza i seguenti componenti:
...
* VMware Cloud (VMC) su AWS con Amazon FSx for NetApp ONTAP
* Un altro ambiente VMware on-premises basato su NFS con storage ONTAP o un ambiente VMFS FC/iSCSI on-premises
