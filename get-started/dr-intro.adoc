...
image:../use/evs-bluexp-architecture.png["Diagramma che mostra i componenti di NetApp Disaster Recovery per VMware"]



== Console NetApp

È possibile accedere a NetApp Disaster Recovery tramite la console NetApp .
...
I volumi protetti possono ospitare numerose VM.  Alcuni potrebbero non far parte di un gruppo di risorse NetApp Disaster Recovery.  In ogni caso, lo spazio di archiviazione utilizzato da tutte le VM su quel volume o LUN viene utilizzato per raggiungere la capacità massima della licenza.
...
Per continuare dopo la prova di 30 giorni, dovrai ottenere un abbonamento Pay-as-you-go (PAYGO) dal tuo provider cloud o acquistare una licenza BYOL da NetApp.
...
* * NetApp Console*: l'interfaccia utente per la gestione dei piani di disaster recovery.  Puoi utilizzare la console NetApp per creare e gestire piani di replica, gruppi di risorse e operazioni di failover nei tuoi ambienti on-premise e cloud.
* *Agente console*: un componente software leggero che viene eseguito nella rete ospitata nel cloud o nell'ambiente VMware locale.  Comunica con la console NetApp e gestisce la replica dei dati tra l'ambiente locale e il sito di disaster recovery.  L'agente Console è installato su una macchina virtuale nel tuo ambiente VMware.
...
* Servizio VMware elastico di Amazon
* Soluzione Azure VMware (AVS) con NetApp Cloud Volumes ONTAP (iSCSI) (anteprima privata)
