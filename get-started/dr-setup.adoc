...
[role="lead"]
Per utilizzare NetApp Disaster Recovery, eseguire alcuni passaggi per configurarlo sia nella NetApp Console che nell'ambiente di ripristino di emergenza supportato.
...
* *AWS Site-to-Site VPN*: Questa connessione di rete privata virtuale può essere utilizzata per fornire connessioni di accesso sicure insieme a un service provider di internet pubblico. La VPN crittografa e decrittografa tutti i dati che transitano da e verso il cloud AWS. Le VPN possono essere basate su software o basate su hardware. Per le applicazioni aziendali, il service provider di internet pubblico (ISP) dovrebbe offrire garanzie di qualità del servizio per assicurare che vengano fornite larghezza di banda e latenza adeguate per la replica di DR.
