...
[role="lead"]
L'account vCenter per NetApp Disaster Recovery deve disporre di un set minimo di privilegi per registrare e annullare la registrazione dei datastore, avviare e arrestare le macchine virtuali (VM) e riconfigurare le VM.

La tabella seguente elenca i privilegi richiesti da Disaster Recovery per funzionare con un vCenter cluster.
...
.2+| Host | Host.Config.AdvancedConfig | Impostazioni avanzate | Consente di impostare opzioni di configurazione host avanzate. 


| Host.Config.Storage | configurazione della partizione di archiviazione | Consente la scansione di nuovi dispositivi di archiviazione e la gestione di iSCSI. 


| Risorsa | Risorsa.AssignVMToPool | Pool di risorse | Consente l'assegnazione di una macchina virtuale a un pool di risorse 
...
.12+| Configurazione della macchina virtuale | VirtualMachine.Config.AdvancedConfig | Configurazione avanzata | Consente di aggiungere o modificare parametri avanzati nel file di configurazione della VM. 


| VirtualMachine.Config.AddExistingDisk | Aggiungi disco esistente | Consente di aggiungere un disco virtuale esistente a una macchina virtuale. 


| VirtualMachine.Config.AddNewDisk | Aggiungi nuovo disco | Consente di creare un nuovo disco virtuale da aggiungere a una macchina virtuale. 


| VirtualMachine.Config.RemoveDisk | Rimuovere il disco | Consente la rimozione di un dispositivo disco virtuale. 
